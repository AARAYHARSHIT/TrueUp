"""Settlement Q&A Agent for TrueUp.

Answers natural-language questions about the reconciliation run by calling
six structured tools against the live pipeline data.  Every factual claim
is backed by a tool result -- the LLM is explicitly forbidden from inventing
numbers.

Tools
-----
get_match_rate()              -> overall + per-pass match statistics
list_exceptions(filter)       -> exception list, optionally filtered by type or source
explain_match(txn_id)         -> detailed explanation for one gateway transaction
summarize(period)             -> high-level narrative for a date range
get_cash_position()           -> unreconciled cash exposure (missing settlements)
get_cash_forecast()           -> projected cash inflows from unreconciled items

Run:
    python -m src.qa_agent               # interactive CLI
    python -m src.qa_agent --test        # run 5 canned test questions and exit
    python -m src.qa_agent "question"    # single question, non-interactive
"""
...
from src.schemas import RecordSource, MatchPass
...
def _load_pipeline() -> dict:
    ...
    det_summary = build_summary(det_matched, det_unmatched, len(gateway), len(bank), len(ledger))
    ...
    exc_report = build_exceptions_report(exceptions, len(gateway), len(bank), len(ledger))
...
def summarize(period: str = "all") -> dict:
    ...
    def in_range(d: Optional[str]) -> bool:
        ...
        try:
            ...
            return start_date <= dt <= end_date  # type: ignore[operator]
        except ValueError:
            return False
...
def run_agent(question: str, verbose: bool = False) -> str:
    """Send a question through the Claude tool-calling loop and return the answer."""
        ...
        if response.stop_reason == "tool_use":
            ...
            for block in response.content:
                if block.type == "tool_use":
                    if verbose:
                        logger.debug("Calling tool: %s(%s)", block.name, json.dumps(block.input))
                    result_json = dispatch_tool(block.name, block.input)
                    if verbose:
                        logger.debug("Tool result: %s", result_json[:300])
                    tool_results.append({
                        "type": "tool_result",
...
# ---------------------------------------------------------------------------
# Test questions (5 canonical)
# ---------------------------------------------------------------------------
TEST_QUESTIONS = [
    "What is the current match rate for this reconciliation run?",
    "List all MISSING_SETTLEMENT exceptions.",
    "Explain what happened to transaction ORD-10071.",
    "Give me a summary of August 2026.",
    "What is the total unreconciled cash position?",
    "How many BATCH_SETTLEMENT exceptions are there?",
    "Explain what happened to transaction ORD-10044.",
    "What exceptions came from the ledger source?",
    "Explain transaction ORD-10057.",
    "What is the deterministic match rate before fuzzy matching?",
    "What is the cash flow forecast for the next two weeks?",
]


def run_tests(verbose: bool = False) -> None:
    """Run 10 canonical test questions and print results."""
    print()
    print("=" * 72)
    print("  TrueUp Q&A Agent -- 10 Test Questions")
    print("=" * 72)
    ...
    print()
    print("=" * 72)
    print(f"  All {len(TEST_QUESTIONS)} test questions answered.")
    print("=" * 72)
    print()
...
def interactive_cli(verbose: bool = False) -> None:
    """Run the interactive question-answering CLI."""
    print("Loading pipeline data...", end=" ", flush=True)
    _load_pipeline()
    print("ready.\n")

    print("TrueUp Settlement Q&A Agent")
    print("Type a question about the reconciliation run.")
    print("Commands: 'quit' / 'exit' / 'q' to leave, 'test' to run 10 test questions.\n")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="TrueUp Settlement Q&A Agent")
    parser.add_argument("--test", action="store_true", help="Run 10 test questions and exit")
    parser.add_argument("--verbose", action="store_true", help="Show debug logs and tool calls")
    parser.add_argument("question", nargs="?", help="Single question to answer (non-interactive)")
    ...
    if args.test:
        ...
        interactive_cli(verbose=args.verbose)
